...
ratings_df |> 
  ...
  slice_head(n = 10) |> 
  ggplot(aes(x = reorder(processing_method, n), y = n, fill = processing_method)) +
  geom_bar(stat = "identity") +
  labs(title = "Top Processing Methods", x = "Processing Method", y = "Count") +
  coord_flip() +
  theme_minimal()
```
...
```{r}
ratings_df |> 
  ggplot(aes(x = species, y = total_cup_points, fill = species)) +
  geom_boxplot() +
  labs(title = "Distribution of Total Cup Points by Species",
       x = "Species", 
       y = "Total Cup Points") +
  theme_minimal()
